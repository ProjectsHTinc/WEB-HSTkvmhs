...
Class Rankmodel extends CI_Model
{
  ...
  function get_rank_details_view($examid,$cls_id,$sub_id,$pass_mark)
  {
   ...
   LEFT JOIN edu_classmaster AS cm ON em.classmaster_id=cm.class_sec_id LEFT JOIN  edu_class AS c
  ...
  function get_all_subject_details($exam_id,$class_id)
  {
      $year_id=$this->getYear();
    // $query1= "SELECT s.subject_name,em.subject_id,em.classmaster_id FROM edu_exam_marks AS em,edu_subject AS s WHERE em.classmaster_id='$clss_id' AND em.subject_id=s.subject_id GROUP By em.subject_id";
     $query1="SELECT s.subject_name,em.subject_id,em.classmaster_id FROM edu_exam_marks as em,edu_subject AS s where exam_id='$exam_id' and classmaster_id='$class_id' and em.subject_id=s.subject_id  group by subject_id";
  ...
  function get_exam_for_class($clss_id){
    $year_id=$this->getYear();
    $query1= "SELECT A.exam_id,A.exam_year,A.exam_name,B.exam_id,B.classmaster_id FROM edu_examination AS A INNER JOIN edu_exam_marks_status AS B ON B.exam_id = A.exam_id and A.exam_year='$year_id' WHERE B.classmaster_id='$clss_id'";
    $sub=$this->db->query($query1);
     if($sub->num_rows()==0){
       $data= array("status" =>"nodata");
       return $data;
  ...
  function get_marks_average($exid,$cls_id,$marktype){
    if($marktype=='Totals'){
       $query="SELECT eem.stu_id,ee.name,sum(eem.total_marks) as total FROM edu_exam_marks as eem left join edu_enrollment as ee on ee.enroll_id=eem.stu_id where eem.classmaster_id='$cls_id'and FIND_IN_SET(eem.exam_id, '$exid') GROUP BY eem.stu_id ORDER BY total desc";
    }else{
      $query="SELECT eem.stu_id,ee.name,avg(eem.total_marks) as total FROM edu_exam_marks as eem left join edu_enrollment as ee on ee.enroll_id=eem.stu_id where eem.classmaster_id='$cls_id'and FIND_IN_SET(eem.exam_id, '$exid') GROUP BY eem.stu_id ORDER BY total desc";
    }
  ...
  function get_exam_name($exid){
    $query="SELECT exam_name FROM edu_examination WHERE FIND_IN_SET(exam_id,'$exid')";
    $sql=$this->db->query($query);
     $res=$sql->result();
     return $res;
